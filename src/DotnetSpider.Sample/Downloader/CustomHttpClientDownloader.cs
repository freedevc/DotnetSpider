...
using DotnetSpider.Sample.FileService;
using DotnetSpider.Sample;
using DotnetSpider.Sample.Extensions;
#if NETFRAMEWORK
using System.Text;
#endif


namespace DotnetSpider.EtechDownloader
{
	/// <summary>
	...
	/// 纯HTTP下载器
	/// </summary>
	public class CustomHttpClientDownloader : DotnetSpider.Downloader.Downloader
	{
		...
		/// <summary>
		/// HttpProxy Pool instance.
		/// </summary>
		private IHttpProxyPool _proxyPool;
		...
		public CustomHttpClientDownloader(IHttpProxyPool proxyPool,IFileStorageService fileService, ProjectDefinition settings,  int timeout = 8000, bool decodeHtml = false)
		{
			_timeout = timeout;
		...
		private bool FileExists(Request request)
		{
			var folder = _settings.DestinationFolder;
			return _fileService.ExistsAsync(folder, request.GetFileName(_settings.FileFormat)).Result;
		}
		protected override Response DownloadContent(Request request)
		{
			...
			try
			{
				...
				{
					if (_proxyPool != null && UseProxy)
					{
						proxy = _proxyPool.GetProxy();
						if (proxy == null)
						{
				...
				if (proxy != null)
				{
					Logger?.LogInformation($"Downloading {request.Url} using Proxy: {proxy.Address}");
				}
				httpResponseMessage =
 ...
 System.Web.HttpUtility.UrlDecode(System.Web.HttpUtility.HtmlDecode(content.ToString()), string.IsNullOrEmpty(request.EncodingName) ? Encoding.UTF8 : Encoding.GetEncoding(request.EncodingName));
			...
			{
				if (!UseFiddlerProxy && _proxyPool != null && proxy != null)
				{
					_proxyPool.ReturnProxy(proxy,
